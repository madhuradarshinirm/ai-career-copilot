...
Version 1.1 | Day 2 Design Output, updated Day 3 | Source of truth: PRD.docx, Implementation_Blueprint_Days2-10.md

**Day 3 change log:** AI provider switched from Anthropic Claude API to **Google Gemini API**. Reason: the PRD (Section 9, Non-Functional Requirements) requires the product to run entirely on free tiers, and the Claude API account in use had $0 available credits. Gemini offers a genuine free tier. This is a like-for-like swap — the AI call remains a single wrapped client behind the backend, so no other architecture, schema, or API contract changed.
...
flowchart TB
    ...
    subgraph AIProvider["Google (Gemini)"]
        Gemini["Gemini API"]
    end
    ...
    API -- "6. AI requests<br/>(gap analysis, feedback)" --> Gemini
    Gemini -- "7. Structured JSON response" --> API
...
Two distinct AI calls exist in v1.0, both using the Gemini API with **strict JSON-only prompting**:
...
- Both calls are stateless from Gemini's perspective — no conversation history is sent, which keeps prompts small, fast, and predictable.
...
| Google Gemini API | Resume analysis + answer feedback | Free tier (switched from Anthropic Claude API on Day 3 — Claude account had $0 credits; Gemini's free tier satisfies the PRD's "must run on free tiers" requirement) |
...
- The Gemini API key lives **only** in the backend's environment variables (Render dashboard) — never shipped to the browser.
