...
All endpoints below are served from the Express backend. Simple reads (dashboard history, session status) happen via **direct Supabase client calls from the frontend** and are not listed here — this file covers only endpoints that require the backend (i.e. anything touching the Gemini API key or shared business logic).
...
**Purpose:** Takes a saved resume and generates the Skill Gap Report + Prep Plan via the Gemini API.
...
| 502 | Gemini API call failed after retry, or returned unparseable JSON |
